...
class GSBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):
        ...
        self.conv_lighting = nn.Sequential(
            GSConv(c1, c_, 1, 1),
            # cbam_block(c_),
            # EfficientLocalizationAttention(c_),
            GSConv(c_, c2, 1, 1, act=False),
            # EfficientLocalizationAttention(c2)
        )
        # for receptive field
    ...
    def forward(self, x):
        ...
        # return self.conv_lighting(x) + self.shortcut(x) + self.Maxpool(x)
...
if __name__ == '__main__':
    ...
    model = VoVGSCSP(256 , 256)  # 创建 ScConv 模型
    print(model(x).shape)  # 打印模型输出的形状
