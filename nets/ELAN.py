...
class Multi_Concat_Block(nn.Module):
    def __init__(self, c1, c2, n=2, e=1, ids=[0]):
        ...
        self.cv3 = nn.ModuleList(
            ...
            [C2f(c_ if i == 0 else c2, c2,shortcut=True) for i in range(n)]
        )

        self.cv4 = Conv(c_ * 2 + c2 * (len(ids) - 2), c2, 1, 1)
...
if __name__ == '__main__':
    ...
    model = Multi_Concat_Block(256,256)  # 创建 ScConv 模型
